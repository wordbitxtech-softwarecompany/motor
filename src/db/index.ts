 ...
 * Rewrite DATABASE_URL / POSTGRES_URL to the shared IPv4 pooler (session :5432).
 */
...
function rewriteDirectToPooler(url: string): string | null {
  ...
  // Prefer aws-0 then aws-1 (same region as your Supabase project — Seoul = ap-northeast-2)
...
function normalizeUrl(url: string): string {
  ...
  // node-pg uses prepared statements — session mode (5432) is safer than transaction (6543)
...
export function resolveDatabaseUrl(): { url: string; source: string; usingPooler: boolean } {
  ...
  if (!chosen) {
    throw new Error(
      "Database URL is missing. On Vercel set DATABASE_URL (or POSTGRES_URL) to the Supabase Session pooler URI (*.pooler.supabase.com:5432), not db.*.supabase.co.",
    );
  }
...
if (process.env.NODE_ENV !== "production") {
  globalForDb.__motorPkPool = pool;
}
