...
export async function POST(req: Request) {
  try {
    ...
    const slug = body.slug || `${body.make}-${body.model}-${body.year}-${Date.now()}`.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    ...
    const [newVehicle] = await db.insert(vehicles).values({
      ...
      mainImage: body.mainImage || 'https://images.pexels.com/photos/34453317/pexels-photo-34453317.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=750&w=1200',
      gallery: body.gallery && Array.isArray(body.gallery) ? body.gallery : [body.mainImage || 'https://images.pexels.com/photos/34453317/pexels-photo-34453317.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=750&w=1200'],
      features: body.features && Array.isArray(body.features) ? body.features : ['Airbags', 'ABS', 'Reverse Camera', 'Smart Key'],
      description: body.description || 'Premium inspected vehicle available at MOTOR Pakistan.',
    }).returning();
